// FILE: src\audio_engine\mod.rs
// =============================

// --- 1. Declare all the new files as sub-modules ---
mod atmo;
mod command;
mod fx_rack;
mod helpers;
...
pub struct AudioEngine {
    ...
    metronome: Metronome,
    metronome_playhead: usize,
    pub synth: Synth,
    sampler_pads: Vec<SamplerPad>,
    atmo_engine: AtmoEngine,
...
impl AudioEngine {
    ...
    pub fn handle_commands(&mut self) {
        while let Some(command) = self.command_consumer.pop() {
            match command {
                AudioCommand::SetTempoState { master_index, multiplier } => {
                    self.master_looper_index.store(master_index, Ordering::Relaxed);
                    self.tempo_multiplier.store(multiplier, Ordering::Relaxed);
                }
                AudioCommand::DoubleTempo => {
                    let current = self.tempo_multiplier.load(Ordering::Relaxed);
                    self.tempo_multiplier.store(current * 2, Ordering::Relaxed);
                }
                AudioCommand::HalveTempo => {
                    let current = self.tempo_multiplier.load(Ordering::Relaxed);
                    self.tempo_multiplier.store(current / 2, Ordering::Relaxed);
                }
                ...
                AudioCommand::ToggleLooperPlayback(id) => {
                    if let Some(looper) = self.loopers.get_mut(id) {
                        ...
                        match state {
                            ...
                            LooperState::Stopped => {
                                looper.play_is_queued = true;
                            }
                            _ => {
                                // Do nothing for empty, armed, recording states
                            }
                        }
                    }
                }
                ...
                AudioCommand::ClearFxRack(insertion_point) => match insertion_point {
                    ...
                    fx::InsertionPoint::Atmo => self.atmo_fx_rack = None,
                },

                AudioCommand::ClearAtmoLayer {
                    scene_index,
                    layer_index,
                } => {
                    if let Some(scene_processor) = self.atmo_engine.scene_processors.get_mut(scene_index) {
                        if let Some(layer) = scene_processor.layers.get_mut(layer_index) {
                            layer.load_samples(Vec::new()); // This also stops all voices
                        }
                    }
                }
                AudioCommand::LoadAtmoLayer {
                    scene_index,
                    layer_index,
                    samples,
                } => {
                    self.atmo_engine.load_layer_samples(scene_index, layer_index, samples);
                }
                AudioCommand::SetAtmoScene {
                    scene_index,
                    scene,
                } => {
                ...
                AudioCommand::LoadLoopAudio {
                    ...
                    length_in_cycles, // CAPTURE THE NEW FIELD
                } => {
                    ...
                    match Self::load_and_resample_wav_for_session(
                        ...
                        Ok(audio_data) => {
                            if let Some(looper) = self.loopers.get_mut(looper_index) {
                                ...
                                // USE THE VALUE FROM THE COMMAND
                ...
                AudioCommand::AdjustParameterRelative { parameter, delta } => {
    ...
    pub fn process_buffer(&mut self, mic_buffer: &mut [f32]) -> Vec<f32> {
        ...
        let musical_bar_len = if transport_len > 0 && multiplier > 0.0 {
            (transport_len as f32 / multiplier) as usize
        } else {
            transport_len
        };
        ...
        let release_coeffs = match self.limiter_release_mode {
            ...
            LfoRateMode::Sync => {
                if musical_bar_len > 0 {
                    ...
                    let release_samples = musical_bar_len as f32 / sync_rate;
        ...
        for i in 0..num_samples {
            ...
            // Metronome logic is now independent of wrapping
            if musical_bar_len > 0 && transport_is_playing {
                let quarter_note_len = musical_bar_len / 4;
                ...
                self.metronome_playhead = (self.metronome_playhead + 1) % musical_bar_len;
            } else {
            ...
            if transport_len == 0 {
                ...
                if let Some(id) = looper_id_to_process {
                    ...
                    if new_len > 0 {
                        ...
                        self.master_looper_index.store(id, Ordering::Relaxed);
            ...
            if sampler_is_active {
                for (pad_idx, pad) in self.sampler_pads.iter_mut().enumerate() {
                    if pad.amp_adsr.state != crate::synth::AdsrState::Idle {
                        playing_mask |= 1 << pad_idx;

                        if pad.playhead as usize >= pad.audio.len() {
                            if pad.amp_adsr.state != crate::synth::AdsrState::Release {
                                pad.amp_adsr.note_off();
                            }
                        }
                    ...
                    LooperState::Playing | LooperState::Overdubbing => {
                        if !looper.audio.is_empty() {
                            ...
                            if transport_is_playing {
                                ...
                                if looper.playhead == 0 && looper.stop_is_queued {
            ...
            if !metronome_state.is_muted && metronome_state.volume > 0.0 {
                metronome_sample = self.metronome.process() * metronome_state.volume;
            }
        ...
        self.atmo_peak_meter.store(
            (atmo_peak_buffer * u32::MAX as f32) as u32,
            Ordering::Relaxed,
        );
    ...
    fn adjust_parameter(&mut self, parameter: crate::settings::ControllableParameter, delta: f32) {
        use crate::settings::ControllableParameter;

        // Helper function to apply a delta to an AtomicU32, assuming a 0.0-1.0 range
        let adjust_atomic = |atomic: &Arc<AtomicU32>| {
            let current_val = atomic.load(Ordering::Relaxed) as f32 / PARAM_SCALER;
            let new_val = (current_val + delta).clamp(0.0, 1.0);
            atomic.store((new_val * PARAM_SCALER) as u32, Ordering::Relaxed);
        };

        // Helper for volumes, which often have a range up to 1.5
        let adjust_atomic_volume = |atomic: &Arc<AtomicU32>| {
            let current_val = atomic.load(Ordering::Relaxed) as f32 / PARAM_SCALER;
            let new_val = (current_val + delta * 1.5).clamp(0.0, 1.5); // Scale delta for larger range
            atomic.store((new_val * PARAM_SCALER) as u32, Ordering::Relaxed);
        };
        ...
        match parameter {
            ...
            // Note: AtmoXY might feel better with absolute control, but we can add it.
            // This is a more complex case and can be added later if needed.

            // We intentionally don't handle non-continuous parameters here.
            _ => {}
        }
    }
}